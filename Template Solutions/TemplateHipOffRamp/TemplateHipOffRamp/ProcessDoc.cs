...
namespace BC.Integration.AppService.TemplateHipOffRamp
{
    ...
    public class ProcessDoc
    {
        ...
        public bool Execute(string receiveMsg)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //Map message to EConnect structure
                    ...
                    try
                    {
                        ...
                        outgoingMessage = Mapper.Convert(msgBody);
                    }
                    catch (Exception ex)
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    instrumentation.LogGeneralException("Error occurred in the BC.Integration.AppService.TemplateService.ProcessDoc.Execute method. " +
                        "The processing of the received message caused the component to fail and processing to stop. Message ID: " + msgID, ex);
                    //throw new Exception("An exception occurred in the BC.Integration.AppService.GP.ProcessSalesDoc Execute method. The processing of the received message caused the component to fail and processing to stop. Message ID: " + msgID, ex);
                    return false;
                }
                finally
                {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("An exception occurred in the BC.Integration.AppService.TemplateService.ProcessDoc.Execute method trying to resolve the Unity DI components.  Please check the service configuration.", ex);
            }
        ...
        private string GetXsltMap()
        {
            try
            {
                ...
                Trace.WriteLineIf(tracingEnabled, tracingPrefix + "Mapping key: " + mapKey);
                //Get XSLT path from configuration using the determined key
                string val = Utilities.GetConfigurationValue(configuration, mapKey);
                Trace.WriteLineIf(tracingEnabled, tracingPrefix + "Mapping value: " + val);
                return val;
            }
            catch (Exception ex)
            {
                Trace.WriteLine(tracingPrefix + "An exception occured trying to resolve the XSLT map path in the BC.Integration.AppService.TemplateService.ProcessDoc.GetXsltMap. Exception message: " + ex.Message);
                throw new Exception("An exception occured trying to resolve the XSLT map path in the BC.Integration.AppService.TemplateService.ProcessDoc.GetXsltMap.", ex);
            }
        }
