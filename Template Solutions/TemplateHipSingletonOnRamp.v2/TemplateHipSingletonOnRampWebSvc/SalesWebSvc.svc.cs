...
namespace BC.Integration.AppService.TemplateHipSingletonOnRamp
{
    ...
    public class ProcessWebSvc : IWebSvc
    {
        public void InitializeProcess(string guid)
        {
            try
            {
                //Start process -LightspeedPosSales
                string processName = "LightspeedPosSales";
                Trace.WriteLine("TemplateHipSingletonOnRamp: We are now in the InitializeProcess method of the TemplateHipSingletonOnRamp Service.  GUID: " + guid);
                ProcessStartInfo startInfo = new ProcessStartInfo();
                startInfo.CreateNoWindow = true;
                startInfo.UseShellExecute = false;
                startInfo.FileName = ConfigurationManager.AppSettings["ExecutableFilePath"];
                startInfo.Arguments = guid + " " + processName;
                Process.Start(startInfo);
            }
            catch (Exception ex)
            {
                throw new Exception("Error occured in the BC.Integration.AppService.TemplateHipSingletonOnRamp.WebSvc.InitializeProcess method.  See inner exception for details.", ex);
            }
        }
