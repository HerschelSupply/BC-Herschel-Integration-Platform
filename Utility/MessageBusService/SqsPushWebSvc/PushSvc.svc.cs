    ...
    public class PushSvc : IPushSvc
    {
        public void InitializeProcess(string guid, string queueName, decimal maxRate, bool isBatch, int maxBatchSize)
        {
            ...
            { 
                ...
                //startInfo.FileName = @"C:\HIP Herschel Integration Platform\Utility\MessageBusService\SqsPushWebSvc.v1\bin\Corp.Integration.Utility.SqsPushService.exe";
                ...
                //Process.Start("Corp.Integration.Utility.SqsPushService.exe " + guid + " " + queueName + " " + maxRate + " " + isBatch + " " + maxBatchSize);
                Process.Start(startInfo);
            }
            catch (Exception ex)
            {
                throw new Exception("Error occured in the Corp.Integration.Utility.SqsPushWebSvc.PushSvc.InitializeProcess method.  See inner exception for details.", ex);
            }
        }
