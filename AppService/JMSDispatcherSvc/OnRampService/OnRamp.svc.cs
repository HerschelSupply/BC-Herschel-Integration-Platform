...
namespace BC.Integration.AppService.JMSDispatcherSvc
{
    ...
    public class OnRamp : IOnRamp
    {
        ...
        public bool InitializeProcess(string guid)
        {
            //Start process
            ...
            try
            {
               return processor.ProcessData(guid);

            }
            catch (Exception ex)
            {
                Trace.WriteLine(tracingPrefix + " service exception occurred. Exception message: " + ex.Message);
