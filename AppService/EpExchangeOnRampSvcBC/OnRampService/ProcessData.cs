...
using BC.Integration.Canonical.SaleChannelOrder.Exchange;
...
namespace BC.Integration.AppService.EpExchangeOnRampServiceBC
{
    public class Process
    {
        ...
        /// <summary>
        /// Process Data is the main method in the BC.Integration.AppService.EpExchangeOnRampServiceBC class.  It implements the following steps;
        ...
        {
            activationGuid = new Guid().ToString();
            try
            {
                CreateDiComponents();
                try
                ...
                {
                    Trace.WriteLine(tracingExceptionPrefix + "An exception was raised when calling the BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessData method. Exception message: " + ex.Message);
                    instrumentation.LogGeneralException("An exception was raised when calling the BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessData method.", ex);
                }
                finally
                {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(tracingExceptionPrefix + "An exception was raised when calling the BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessData method trying to resolve the Unity DI components. Exception message: " + ex.Message);
                throw new Exception("An exception occured in the BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessData method trying to resolve the Unity DI components.", ex);
            }
        }
        ...
        {
            String docId = "";
            Trace.WriteLineIf(tracingEnabled, tracingPrefix + " BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessFile.  Start Processing File.");
            ...
            Trace.WriteLineIf(tracingEnabled, tracingPrefix + " BC.Integration.AppService.EpExchangeOnRampServiceBC.ProcessData.  Convert XML message to an object.");
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLineIf(tracingEnabled, tracingPrefix + " BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.ProcessFile.  The creation of the canonical message failed. The message wasn't sent to the queue. DocumentId: " + docId + " Exception message: " + ex.Message);
        ...
        /// Order = CanonicalSalesChannelOrder,  ExchangeOrder.Order = JMS Object
        /// </summary>
        /// <param name="salesOrder"></param>
        /// <returns></returns>
        private Order MapSalesOrdersToCanonical(ExchangeOrder.Order salesOrder)
        {
            try
            {
                ...
                foreach (ExchangeOrder.Promotion promos in salesOrder.AppliedPromotions.Promotion)
                {
                ...
                {
                    order.LineItems = new List<OrderLineItem>();
                    foreach (ExchangeOrder.LineItem so_item in salesOrder.Shipments.Shipment.LineItems.LineItem)
                    {
                ...
                else
                {
                    ...
                    foreach (ExchangeOrder.Address so_item in salesOrder.Addresses.Address)
                    {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("An error occured while trying to map the EP sales order message to the SalesChannelOrder message " +
                    "in the BC.Integration.AppService.EpExchangeOnRampServiceBC.Process.MapSalesOrdersToCanonical method. The EP sales order transaction " +
