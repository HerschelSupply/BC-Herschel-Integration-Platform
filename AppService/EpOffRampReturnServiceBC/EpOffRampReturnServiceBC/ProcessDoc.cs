...
namespace BC.Integration.AppService.EpOffRampReturnServiceBC
{
    ...
    public class ProcessDoc
    {
        ...
        public bool Execute(string receiveMsg)
        {
            ...
            try
            {
                ...
                try
                {
                   ...
                   // final.LoadXml(msgBody);

                   // string jsonText = JsonConvert.SerializeXmlNode(final);
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("An exception occurred in the BC.Integration.AppService.EpOffRampReturnServiceBC.ProcessDoc.Execute method trying to resolve the Unity DI components.  Please check the service configuration.", ex);
            }
        ...
        public string CreateBCMsg(string msgBody)
        {
            ...
            {
                throw new Exception("An exception occured implementing the XSLT mapping in the BC.Integration.AppService.EpOffRampReturnServiceBC.", ex);
            }
        }
        ...
        private string GetXsltMap()
        {
            ...
            {
                Trace.WriteLine(tracingPrefix + "An exception occured trying to resolve the XSLT map path in the BC.Integration.AppService.EpOffRampReturnServiceBC.ProcessDoc.GetXsltMap. Exception message: " + ex.Message);
                throw new Exception("An exception occured trying to resolve the XSLT map path in the BC.Integration.AppService.EpOffRampReturnServiceBC.ProcessDoc.GetXsltMap.", ex);
            }
        }
